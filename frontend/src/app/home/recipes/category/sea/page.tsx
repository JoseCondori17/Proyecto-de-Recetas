...
export default function Sea() {
  ...
  const fetchRandomRecipes = async () => {
    try {
      const response = await fetch('https://spoonacular-recipe-food-nutrition-v1.p.rapidapi.com/recipes/random?tags=sea%2Cfish%2Cshrimps&number=15', {
        method: 'GET',
        headers: {
  ...
  return (
    <main className='flex flex-col space-y-4 select-none sm:space-y-6 md:space-y-8'>
      <h1 className='text-2xl font-bold sm:text-3xl md:text-4xl'>Healthy</h1>
